...
$(document).ready(function () {
    ...
    */
    ...
    // line3 25-30 will work, but it requires the user to click off the text box before enabling the button, not good design!
    ...
    function enableBtn() { //  This function will remove the 'disabled' attribute from the submit button, thus enabling the button
        $("#btnSubmit").removeAttr("disabled");
    }
    ...
    // However, non-input keys, such as caps lock or the arrow keys, will 'trick' this into firing despite there being no input in the box

    // QUESTION -  how else could i check if there is text in the box that is ready for submission?
    ...
    $('#btnSubmit').click(function (event) {
        ...
        */
        ...
        myLi.debounceId = 0; // enables single vs double click functionality

        myLi.addEventListener("click", function (event) {
            if (myLi.debounceId) { // this if statement allow us to get around treating the double click as two single clicks
            ...
            if (event.detail === 1) { // on single click, do this
                myLi.debounceId = setTimeout(function () { // waits 500ms to decide if single or double click
                    ...
                    myLi.style.backgroundColor = randomColor(); // sets a random background color
                    myLi.style.borderRadius = randomBorderRadius(); // sets a random border radius on each corner

                }, 500); // 500ms before executing the single click functionality
            } else if (event.detail === 2) { // on double click, do this
                $(this).remove(); // removes the list item
            }
        });
        myUl.appendChild(myLi); // appends the list item to the list
    });
    ...
    */
    ...
    //and just for fun:
    myUl.onmouseover = function () { // this changes the background color and border radius of the list element on mouse over
        $("ul").css("background-color", randomColor()); // calls the randomColor function
        $("ul").css("border-radius", randomBorderRadius()); // calls the randomBorderRadius function
    };
